...
class Pack:
    ...
    # 默认选项
    __spec__ = Path("static/pack/__spec__")
    ...
    def __init__(self,
        ...
        if self.__os__ == "Windows":
            ...
            if icon:
                ...
                self.__argv__.append("--icon={}".format(icon))
        ...
        self.__argv__.append(script)
        print(self.__argv__)
        self.__execute__()

    def __execute__(self):
        print(" ".join(self.__argv__))
        subprocess.Popen(" ".join(self.__argv__)).communicate()

    def __datas__(self, datas:List[Path]):
        targets = []
        for source in datas:
            source = self.__check_path__(source)
            targets.append( "--add-data {}".format(self.__sep__.join((str(source.absolute()), str(source)))))
        return " ".join(targets)
    ...
    def __binaries__(self, binaries:List[Path]):
        ...
        for source in binaries:
            ...
            targets.append("--add-binary={}".format(self.__sep__.join((str(source.absolute()), str(source)))))
        return " ".join(targets)
    ...
    def help():                     # -h, --help: 显示帮助信息
        return subprocess.Popen("pyinstaller --help")
    
    @staticmethod
    def version():                  # -v, --version: 显示版本信息
        return subprocess.Popen("pyinstaller --version")
    
    @staticmethod
    def clean(path):                    # --clean: 清理临时文件并退出
        return subprocess.Popen("pyinstaller --clean {}".format(Pack.__check_path__(path, check=True))).communicate()
...
if __name__ == "__main__":
    Pack("test.py",
         icon="static/icons/redis.ico",
         datas=[
             "static/icons/redis.ico",
    ...
    # s = Path("static/icons/redis.ico").absolute().relative_to(Path.cwd())
